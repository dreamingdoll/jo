def func_a(current_grade, last_grade, rank, max_diff_grade):
    arr_length = len(current_grade) # arr_length 의 길이 = current_list의 길이
    count = 0
    for i in range(arr_length): # arr_length의 길이만큼 반복
        if current_grade[i] >= 80 and rank[i] <= arr_length // 10: # 이번학기 성적이 80점 이상이면서 석차가 상위 10% 이내인 학생
            count += 1 # 장학생 + 1
        elif current_grade[i] >= 80 and rank[i] == 1: # 이번학기 성적이 80점 잇강이면서 석차가 1등인 학생
            count += 1 # 장학생 + 1
        elif max_diff_grade > 0 and max_diff_grade == current_grade[i] - last_grade[i]: # (이번학기 성적 - 직전학기 성적 )이  양수이고 그 값이 func_c의 값과 같으면
            count += 1 # 장학생 + 1
